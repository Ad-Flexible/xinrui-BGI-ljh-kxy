...
public class SampleServiceImpl implements SampleService {
    ...
    public SampleDto getSample(String sampleNum) {
        log.info("查询样本信息，原样本编号: {}", sampleNum);

        // 1. 查询数据库（优先使用唯一索引字段）
        SampleDto sampleDto = sampleMapper.selectBySampleNum(sampleNum);
        if (sampleDto == null) {
            log.warn("样本信息不存在，原样本编号: {}", sampleNum);
